...
export default function DashboardView({ metrics }: { metrics: DashboardMetrics }) {
    const STATS = [
        { name: "Total Blogs", value: metrics.totalBlogs.toString(), change: "+4", icon: FileText, color: "text-blue-500", bg: "bg-blue-500/10" },
        {
            name: "Total Views",
            value: metrics.totalViews >= 1000 ? `${(metrics.totalViews / 1000).toFixed(1)}K` : metrics.totalViews.toString(),
            change: "+12%",
            icon: Eye,
            color: "text-purple-500",
            bg: "bg-purple-500/10"
        },
        { name: "Subscribers", value: metrics.subscribers.toLocaleString(), change: "+0", icon: Users, color: "text-green-500", bg: "bg-green-500/10" },
        { name: "Avg. Read Time", value: metrics.avgReadTime, change: "-2%", icon: TrendingUp, color: "text-orange-500", bg: "bg-orange-500/10" },
    ];
                ...
                {STATS.map((stat, index) => (
                    ...
                    >
                        <div className="flex items-center justify-between mb-4">
                            ...
                            {/* Placeholder change indicator logic can be enhanced later with historical data */}
